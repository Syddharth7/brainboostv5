-- Complete SQL script to insert quizzes linked to topics (Auto-generates UUIDs)
-- Run this AFTER the migration script has been run

-- First, delete quiz questions for these specific quiz IDs
...
-- Then delete the quizzes themselves
...
-- Insert quizzes for each topic with proper topic_id references
-- Topic 1: Computer Hardware Fundamentals
INSERT INTO public.quizzes (id, topic_id, title, passing_score, created_at)
SELECT 
    ...
    now()
FROM public.topics 
WHERE lesson_id = '2168e84c-dc22-44d5-b0d3-d31727155fdf' AND "order" = 1;

-- Topic 2: Software and Operating Systems
INSERT INTO public.quizzes (id, topic_id, title, passing_score, created_at)
SELECT 
    ...
    now()
FROM public.topics 
WHERE lesson_id = '2168e84c-dc22-44d5-b0d3-d31727155fdf' AND "order" = 2;

-- Topic 3: Internet and Networking Basics
INSERT INTO public.quizzes (id, topic_id, title, passing_score, created_at)
SELECT 
    ...
    now()
FROM public.topics 
WHERE lesson_id = '2168e84c-dc22-44d5-b0d3-d31727155fdf' AND "order" = 3;

-- Topic 4: Digital Safety and Security
INSERT INTO public.quizzes (id, topic_id, title, passing_score, created_at)
SELECT 
    'a4444444-4444-4444-4444-444444444444'::uuid,
    id,
    'Digital Safety and Security Quiz',
    70,
    now()
FROM public.topics 
WHERE lesson_id = '2168e84c-dc22-44d5-b0d3-d31727155fdf' AND "order" = 4;

-- Topic 5: Productivity Software and Tools
INSERT INTO public.quizzes (id, topic_id, title, passing_score, created_at)
SELECT 
    'a5555555-5555-5555-5555-555555555555'::uuid,
    id,
    'Productivity Software Quiz',
    70,
    now()
FROM public.topics 
WHERE lesson_id = '2168e84c-dc22-44d5-b0d3-d31727155fdf' AND "order" = 5;

-- Now insert quiz questions for each quiz
...
SELECT q.id, q.title, q.topic_id, t.title as topic_title, t."order", COUNT(qq.id) as question_count
...
GROUP BY q.id, q.title, q.topic_id, t.title, t."order"
ORDER BY t."order";
